...
import { ChevronDown, FolderOpen, ChevronUp } from "lucide-react";
...
export default function CoursesDashboard() {
  ...
  const [activeCourse, setActiveCourse] = useState<{year: string, courseId: string} | null>(null);
  ...
  const handleCourseClick = (year: string, courseId: string, event: React.MouseEvent) => {
  ...
  return (
    <div className="bg-white p-6 min-h-screen space-y-8" onClick={handleDocumentClick}>
                  ...
                  <div key={course.id} className="relative">
                    <div
                      ...
                      className={`bg-gray-50 rounded-lg p-4 min-w-[300px] flex items-center justify-between cursor-pointer transition-all duration-200 border-2 ${
                        activeCourse?.year === year && activeCourse?.courseId === course.id
                          ? "border-amber-500 bg-amber-50"
                          : "border-transparent hover:border-gray-200"
                      }`}
                      aria-expanded={activeCourse?.year === year && activeCourse?.courseId === course.id}
                    >
                      ...
                      <ChevronDown
                        ...
                        className={`text-blue-900 transition-transform duration-200 ${
                          activeCourse?.year === year && activeCourse?.courseId === course.id ? "rotate-180" : ""
                        }`}
                      />
                    </div>
                      ...
                      >
                        {dropdownOptions.map((option) => (
                          <Link
                            ...
                            className="block py-3 px-4 hover:bg-amber-50 text-gray-800 font-medium border-b border-gray-100 last:border-b-0 transition-colors duration-150"
                          >
                            {option}
                          </Link>
                        ))}
